...
function deleteComment(list, token, deleteFetch, reload) {
    ...
    for(let key of listItems) {
        ...
        deleteButton.addEventListener ('click',(event) => {
              ...
              let apiAdress=`https://webdev-hw-api.vercel.app/api/v2/marat-rizatdinov/comments/${key.dataset.index}`;         
...
function likeComment(list, token, likeFetch, reload) {
  ...
  for(let key of listItems) {
      ...
      likeButton.addEventListener ('click',(event) => {
            ...
            let apiAdress=`https://webdev-hw-api.vercel.app/api/v2/marat-rizatdinov/comments/${key.dataset.index}/toggle-like`;         
